...
| Sampling | 固定问题 batch，跳过退化组 | Dynamic Sampling，补采到有效组数量达标 |
...
参考 `05-retool`，保留清晰模块边界，不拆成过多小文件：

```text
06-dapo/
├── prepare_data.py   # 清洗、去重、切分 DAPO-Math-17K
├── data.py           # 数据加载和循环游标
...
训练主循环保持同步；`rollout.py` 内使用 `sample_async` 并发采样，模式与 `05-retool` 一致。不再维护两套重复的 sync/async 训练脚本。
...
PyTRIO 0.2.4 会把 custom loss 的 `-dL/dlogprob` 转成 cross-entropy weights，并在远端按非零 weight token 数归一化。为避免 loss 被二次缩小，custom callback 返回给 PyTRIO 的代理 loss 乘以非零梯度 token 数；`ppo/loss` 指标仍记录上面的原始 sample/token reduction 结果。
...
长度使用 `len(sequence.tokens)`，即与训练 Datum 相同的实际生成 token；不能按字符数计算，也不能只在 `stop_reason` 表示截断时启用。当前 `max_tokens=12288`、`overlong_cache=2048`，所以 10240 token 前不惩罚，11264 token 为 `-0.5`，12288 token 为 `-1`。
...
- 训练数据固定使用 DAPO-Math-17K，从 `05-retool/datasets/raw/dapo-math-17k.parquet` 重新生成，不使用已经重复约 100 倍的 `05-retool/datasets/train.jsonl`。当前实测从 1,791,700 行中得到 17,176 道无冲突唯一题目。
...
- Reward 复用 `05-retool/reward.py` 的 `math_verify + 最后一个 \boxed{}` 规则。
- 模型：`Qwen/Qwen3.5-4B`，LoRA rank 32。
- Adam：lr `4e-5`，betas `0.9/0.95`。
- `group_size=8`；GRPO 每步固定采样 16 组，DAPO 每步补采到 16 个有效组。
- prompt 最多 4095 token，completion 最多 12288 token，`overlong_cache=2048`。
...
- custom loss 经 PyTRIO 权重转换后的梯度缩放。
- Soft Overlong Punishment 在 10240/12288 等区间边界的值。
- Dynamic Sampling 对全对、全错、混合组，以及“全错但 shaped reward 不同”组的处理。

先分别执行 GRPO/DAPO 的 1-step smoke test，再运行正式对照。端到端 smoke 使用正式的 `max_tokens=12288`；`1024` 只适合检查接口连通性，可能在生成 `\boxed{}` 前截断并使整组退化。除相同训练 step 外，还要比较相同 rollout token 数和 wall time，避免 Dynamic Sampling 的额外采样成本被隐藏。

主要指标：

- base/shaped reward、length penalty、accuracy、有效组比例；
...
4. 完成 AIME25 `eval.py`，确认 base model 与 checkpoint 使用同一评测配置。
