    ...
    "function": {
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "label": {"type": "string", "description": "Optional spectral feature label"},
            },
...
def render_messages(messages, tokenizer, generation=False):
    return tokenizer.apply_chat_template(
        messages, tools=TOOLS, chat_template=TEMPLATE, tokenize=False,
        add_generation_prompt=generation, enable_thinking=True,
    )
...
def image_chunk(path, image_processor):
    ...
    patches = image_processor.get_number_of_image_patches(
        image.height, image.width, images_kwargs={},
    )
...
def encode_messages(messages, image_paths, tokenizer, image_processor, generation=False):
    encoded = tokenizer.apply_chat_template(
        messages, tools=TOOLS, chat_template=TEMPLATE,
        tokenize=True, return_dict=True, return_assistant_tokens_mask=True,
        add_generation_prompt=generation, enable_thinking=True,
    )
    return pack_chunks(
        encoded["input_ids"], encoded["assistant_masks"], image_paths,
...
def build_sft_datum(row, tokenizer, image_processor):
    model_input, targets, weights = encode_messages(
        row["messages"], row["images"], tokenizer, image_processor,
    )
    ...
    return trio.Datum(
        ...
        loss_fn_inputs={"target_tokens": targets[1:], "weights": weights[1:]},
    )
...
def tool_continuation(messages, image_paths, tokenizer, image_processor):
    ...
    return pack_chunks(ids, [0] * len(ids), image_paths, tokenizer, image_processor)[0]


def parse_action(text):
    """动作语法属于环境规则；非法动作结束该轨迹，不重试或补写。"""
    body = text.partition("</think>")[2].strip().removesuffix("<|im_end|>").strip()
    answer = re.fullmatch(r"<answer>\s*\\boxed\{(YES|NO)\}(.*?)</answer>", body, re.S)
    if answer:
        return {"kind": "answer", "prediction": answer[1]}
    call = re.fullmatch(
        r"<tool_call>\s*<function=spectral_visualization_tool>\s*(.*?)\s*</function>\s*</tool_call>",
        body, re.S,
    )
    if call:
        parameters = dict(re.findall(r"<parameter=(\w+)>\s*(.*?)\s*</parameter>", call[1], re.S))
        interval = re.fullmatch(
            rf"\[\s*({NUMBER})\s*,\s*({NUMBER})\s*\]", parameters.get("wavelength_range", ""),
        )
        if interval:
            return {
                "kind": "tool", "wavelength_range": [float(interval[1]), float(interval[2])],
