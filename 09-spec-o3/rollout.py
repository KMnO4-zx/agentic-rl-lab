...
async def rollout(
    row, sampler, tokenizer, image_processor, output_dir,
    max_turns=8, max_tokens=2048, max_seq_len=16384, temperature=0.6, seed=42,
):
    output_dir.mkdir(parents=True, exist_ok=True)
    messages = deepcopy(row["messages"])
    prompt, _, _ = encode_messages(messages, row["images"], tokenizer, image_processor, generation=True)
    with np.load(row["spectrum"]) as spectrum:
        wavelength, flux, redshift = spectrum["wavelength"], spectrum["flux"], float(spectrum["redshift"])
    ...
    for turn in range(max_turns):
        ...
        response = await sampler.sample_async(
            ...
            sampling_params=trio.SamplingParams(
                max_tokens=min(max_tokens, remaining), temperature=temperature,
                top_p=0.95, stop=[im_end], seed=seed + turn,
            ),
        )
        ...
        prompt = trio.ModelInput(chunks=[*prompt.chunks, trio.types.EncodedTextChunk(tokens=sequence.tokens)])
        target_tokens.extend(sequence.tokens)
        logprobs.extend(sequence.logprobs)
        action_mask.extend([1.0] * len(sequence.tokens))
        record = {
            "turn": turn, "text": text, "tokens": sequence.tokens,
            "logprobs": sequence.logprobs, "stop_reason": sequence.stop_reason, "action": action,
        }
        turns.append(record)
        ...
        observation = spectral_visualization_tool(
            wavelength, flux, redshift, action["wavelength_range"], action["label"],
        ...
        messages.append({
            ...
            "content": content.removesuffix("<|im_end|>").strip(), "tool_calls": [],
        })
        messages.append({"role": "tool", "content": observation["content"]})
        continuation = tool_continuation(messages, observation["images"], tokenizer, image_processor)
        ...
        context = trio.ModelInput(chunks=[
            trio.types.EncodedTextChunk(tokens=tokenizer.encode(separator, add_special_tokens=False)),
            *continuation.chunks,
        ])
    ...
    return {
        "id": row["id"], "task": row["task"], "label": row["label"],
        "prediction": prediction, "format_ok": finish_reason == "answer",
        "finish_reason": finish_reason, "turns": turns,
        "initial_messages": row["messages"], "initial_images": row["images"],
        "model_input": prompt, "target_tokens": target_tokens,
        "logprobs": logprobs, "action_mask": action_mask,
    }
