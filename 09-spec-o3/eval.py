"""用同一套工具与预算评测 Base、SFT 或后续 RL 的 sampler 权重。"""
...
def classification_metrics(results):
    ...
    for task in sorted({row["task"] for row in results}):
        ...
        tp = sum(row["label"] == "YES" and row["prediction"] == "YES" for row in rows)
        fp = sum(row["label"] == "NO" and row["prediction"] == "YES" for row in rows)
        fn = sum(row["label"] == "YES" and row["prediction"] != "YES" for row in rows)
        # 没有预测正类时 precision 记 0；这是分类指标的 zero-division 定义。
        metrics[task] = {
            "count": len(rows), "tp": tp, "fp": fp, "fn": fn,
    ...
    observations = [turn["observation"] for row in results for turn in row["turns"] if "observation" in turn]
    return {
        "tasks": metrics,
...
async def main(args):
    ...
    sampler = service.create_sampling_client(base_model=BASE_MODEL, model_path=args.model_path)
    args.output.mkdir(parents=True, exist_ok=True)
    results = []
    with (args.output / "trajectories.jsonl").open("w") as file:
        for start in range(0, len(rows), args.concurrency):
            batch = rows[start:start + args.concurrency]
            trajectories = await asyncio.gather(*(
                rollout(
                    row, sampler, tokenizer, processor, args.output / "images" / row["id"],
                    max_turns=args.max_turns, max_tokens=args.max_tokens,
                    max_seq_len=args.max_seq_len, temperature=args.temperature, seed=args.seed + start + index,
                )
                for index, row in enumerate(batch)
            ))
            for result in trajectories:
                # PNG 单独保存；JSONL 保留逐轮文本、原始采样 token/logprob 和动作 mask。
                record = {key: value for key, value in result.items() if key != "model_input"}
                file.write(json.dumps(record, ensure_ascii=False) + "\n")
                results.append({key: record[key] for key in ("task", "label", "prediction", "format_ok", "turns")})
            file.flush()
            print(f"evaluated={len(results)}/{len(rows)}")
    metrics = classification_metrics(results)
    config = {**vars(args), "data": str(args.data), "output": str(args.output), "base_model": BASE_MODEL}
    (args.output / "metrics.json").write_text(json.dumps({"config": config, **metrics}, ensure_ascii=False, indent=2))
    print(json.dumps(metrics, ensure_ascii=False, indent=2))


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--data", type=Path, default=Path(__file__).parent / "datasets/rl/bench_dev.jsonl")
    parser.add_argument("--model-path", help="留空评测 Base；SFT/RL 传入 sampler_path")
